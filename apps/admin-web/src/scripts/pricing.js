...
function filters() {
  ...
  return `<form class="admin-filter-bar admin-order-filter-bar" data-pricing-filter><div class="admin-filter-bar__search"><label class="admin-search-field">${icon("search")}<input class="admin-form-control" name="q" placeholder="Tìm SKU hoặc tên sản phẩm"></label></div><div class="admin-filter-bar__filters"><select class="admin-form-control" name="category"><option value="">Tất cả danh mục</option>${categories.map((value) => `<option>${escapePricingHtml(value)}</option>`).join("")}</select><select class="admin-form-control" name="status"><option value="">Tất cả trạng thái giá</option><option value="discount">Đang giảm giá</option><option value="invalid">Cần kiểm tra</option><option value="missing">Thiếu giá bán</option></select></div><div class="admin-filter-bar__actions"><button class="admin-btn admin-btn--filter admin-btn--sm" type="submit">Lọc</button><button class="admin-btn admin-btn--ghost admin-btn--sm" type="reset">Đặt lại</button></div></form>`;
}
function pagination() {
