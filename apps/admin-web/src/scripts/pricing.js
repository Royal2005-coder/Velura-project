...
function filters() {
  ...
  return `<form class="admin-filter-bar admin-order-filter-bar" data-pricing-filter><label class="admin-search-field">${icon("search")}<input class="admin-form-control" name="q" placeholder="Tìm SKU hoặc tên sản phẩm"></label><select class="admin-form-control" name="category"><option value="">Tất cả danh mục</option>${categories.map((value) => `<option>${escapePricingHtml(value)}</option>`).join("")}</select><select class="admin-form-control" name="status"><option value="">Tất cả trạng thái giá</option><option value="discount">Đang giảm giá</option><option value="invalid">Cần kiểm tra</option><option value="missing">Thiếu giá bán</option></select><button class="admin-btn admin-btn--filter admin-btn--sm" type="submit">Lọc</button><button class="admin-btn admin-btn--ghost admin-btn--sm" type="reset">Đặt lại</button></form>`;
}
function pagination() {
